...
| `ORDER_PROVIDER` | `none` | `none`, `mock`, `fenecan` | De onde chegam pedidos. `fenecan` = builds do site via fenecan-backend (`interligando/`). `mock` é **apenas para desenvolvimento**. |
| `FENECAN_WS_URL` | `ws://localhost:3000/ws/os` | URL `ws://` | WebSocket da O.S. no fenecan-backend (com `fenecan`). |
| `OS_HTTP_PORT` | `4100` | 1–65535 | API local (`GET /orders`, `GET /orders/:id`, `PATCH /orders/:id/status`) usada pela tela do operador. |
| `VITE_OS_API_URL` | `http://127.0.0.1:4100` | URL | Endereço dessa API, lido pela tela (`web/`, Vite). |
...
> **Ligado:** a API oficial é o fenecan-backend (`interligando/fenecan-backend`).
> `ORDER_PROVIDER=fenecan` usa `FenecanOrderProvider` + `FenecanBuildMapper`
> (em `src/infrastructure/integrations/external-api/fenecan/`), que recebem as
> builds pelo WebSocket `/ws/os`. Os passos abaixo continuam valendo para
> qualquer outra origem.

Para uma nova origem:
...
- **Tela do operador:** a tela React (`web/`, aberta pelo Tauri) usa a API local deste backend (`src/infrastructure/http/os-http-server.ts`). O sistema operador antigo (`src/main.ts`, `src/ui/`) não faz parte do fluxo integrado e continua como estava.
- **Status não volta para a fila:** o fluxo de status da O.S. (operador) e o da fila da Unity (`WAITING → BUILDING → COMPLETED/ERROR`, no fenecan-backend) são independentes. O provedor recebe `BUILD_UPDATED`, mas não o aplica; a tabela de nomes acima é o ponto de partida para decidir isso.
- **Backend em memória reiniciado:** com `BUILD_STORAGE=memory` no fenecan-backend, reiniciá-lo recomeça em `BUILD-001`, e essas builds colidem com O.S. já existentes. A O.S. avisa no terminal (`[fenecan] ATENCAO`); reinicie as duas juntas ou use MySQL.
